...
class GigaChat:
    ...
    def send_request(self, user_message):
        ...
        try:
            response = requests.post(
                self.api_url, headers=headers, data=payload, verify=False
            )
    ...
    def create_request(self):
        ...
        general_user_info_json = general_user_info.to_json(
            orient="records", force_ascii=False
        )
        genders_friends_json = genders_friends.to_json(
            orient="records", force_ascii=False
        )
        cities_friends_json_1 = cities_friends.head(len(cities_friends) // 3).to_json(
            orient="records", force_ascii=False
        )
        cities_friends_json_2 = cities_friends.tail(len(cities_friends) // 3).to_json(
            orient="records", force_ascii=False
        )
        ...
        responses["general_user_info"] = self.send_request(
            f"Сделай краткую выжимку о пользователе ВК по переданным данным, которые содержат информацию об общей информации пользователя:\n{general_user_info_json}"
        )
        responses["age_friends"] = self.send_request(
            f"Статистика распределения друзей пользователя по возрасту. Не составляй таблицу:\n{age_friends_json}"
        )
        responses["genders_friends"] = self.send_request(
            f"Статистика распределения друзей пользователя по полу:\n{genders_friends_json}"
        )
        responses["cities_friends_1"] = self.send_request(
            f"Статистика распределения друзей пользователя по городам. Составь список с этой информацией, не составляя таблицу:\n{cities_friends_json_1}"
        )
        responses["cities_friends_2"] = self.send_request(
            f"Статистика распределения друзей пользователя по городам. Составь список с этой информацией, не составляя таблицу:\n{cities_friends_json_2}"
        )
        responses["stats"] = self.send_request(
            f"Активность пользователя:\n{stats_json}"
        )
        responses["marks"] = self.send_request(f"Статистика со стены:\n{marks_json}")
        responses["interests"] = self.send_request(
            f"Интересы пользователя:\n{interests_json}"
        )
        responses["toxicity"] = self.send_request(
            f"Оценка токсичности пользователя по переданным данным, которые содержат вероятности нетоксичности, грубость, непристойности, агрессивности, опасности пользователя. Если нет значений, то написать, что нет сведений о токсичности:\n{toxicity_json}"
        )
